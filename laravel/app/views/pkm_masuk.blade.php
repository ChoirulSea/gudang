...
@section('content')
    ...
    <div class="page-content-wrap">
        ...
        <div class="row">
            <div class="col-md-12">
                <form class="form-horizontal" method="post" id="subm" action="{{route('pkm-masuk')}}">
                    <div class="panel panel-default @if(!$errors->has()) panel-toggled @endif" id="daftar">
                        ...
                        <div class="panel-body">
                            ...
                            <div class="form-group">
                                <label class="col-md-2 col-xs-12 control-label">Unit Pengadaan</label>
                            ...
                            <div class="form-group">
                                <label class="col-md-2 col-xs-12 control-label">Jenis Barang</label>

                                <div class="col-md-7 col-xs-12">
                                    <select class="form-control select" name="am">
                                        <option value="habis pakai">Habis Pakai</option>
                                        <option value="aset">ASET</option>
                                    </select>
                                </div>
                            </div>
                            <div class="form-group hidden" id="menu_aset">
        ...
        $(document).ready(function () {
            ...
            $('#add').click(function () {
                ...
                if (brg == '') {
                    ...
                    '<td><input type="text" class="hidden" readonly name="ppn[]" value="' + tot + '"><label class="control-label pull-right">' + tot + '%</label></td>' +
                    ...
                    $('[name=ppn]').val('');
                    $('[name=addbrg]').val('');
                    $('[name=am]').prop('disabled', 'disabled');
                    $('.select').selectpicker('refresh');
                }
            });
            ...
            $('[name=am]').change(function () {
                var ab = $(this).val();
                if (ab == 'aset') {
                    $('#menu_aset').removeClass('hidden');
                    $('#menu_hp').addClass('hidden');
                } else {
                    $('#menu_aset').addClass('hidden');
                    $('#menu_hp').removeClass('hidden');
                }
            });
            ...
            $('[id^=form]').on('submit', function (e) {
                ...
                $.ajax({
                    ...
                    success: function (result) {
                        ...
                        $('#body-table').append('<tr class="success" style="font-size: 10pt">' +
                        '<td><input type="text" class="hidden" readonly name="barang[]" value="' + result.id + '"><label class="control-label">' + result.id_aset + ' - ' + brg + '</label></td>' +
                        '<td><label class="control-label pull-right">' + jum + '</label></td>' +
                        '<td><label class="control-label pull-right">' + sat + '</label></td>' +
                        '<td><label class="control-label">' + h_sat + '</label></td>' +
                        '<td><label class="control-label pull-right">' + ppn + '%</label></td>' +
                        '<td><a onclick="$(this).parent().parent().remove();" class="btn btn-icon-only red"><i class="fa fa-times"></i></a></td>' +
                        '</tr>');
                    }
                });
                $('#' + f + ' select.form-control').val('');
        ...
        function pop_editt(id) {
            ...
            $.ajax({
                ...
                success: function (result) {
                    ...
                    $('[name=am]').val(result.data['barang']);
                    $('[name=am]').prop('disabled', 'disabled');
                    $('.select').selectpicker('refresh');
                    if (result.data['barang'] == 'aset') {
                        $('#menu_aset').removeClass('hidden');
                        $('#menu_hp').addClass('hidden');
                    } else {
                        $('#menu_aset').addClass('hidden');
                    ...
                    for (i = 0; i < result.dtl.length; i++) {
                        if (result.data['barang'] == 'aset') {
                            if (result.cek == 0) {
                                var dlt = '<td><a onclick="$(this).parent().parent().remove();" class="btn btn-icon-only red"><i class="fa fa-times"></i></a></td>';
                            } else {
                                var dlt = '<td>&nbsp;</td>';
                            }
                            $('#body-table').append('<tr class="success" style="font-size: 10pt">' +
                            '<td><input type="text" class="hidden" readonly name="barang[]" value="' + result.dtl[i]['id'] + '"><label class="control-label">' + result.dtl[i]['id_aset'] + ' - ' + result.dtl[i]['nama'] + '</label></td>' +
                            '<td><label class="control-label">' + result.dtl[i]['jml'] + '</label></td>' +
                            '<td><label class="control-label pull-right">' + result.dtl[i]['satuan'] + '</label></td>' +
                            '<td><label class="control-label">' + result.dtl[i]['h_satuan'] + '</label></td>' +
                            '<td><label class="control-label pull-right">' + result.dtl[i]['ppn'] + '%</label></td>' + dlt + '</tr>');
                        } else {
                            $('#body-table').append('<tr class="success" style="font-size: 10pt">' +
                            ...
                            '<td><input type="text" class="hidden" readonly name="ppn[]" value="' + (result.dtl[i]['ppn']) + '"><label class="control-label pull-right">' + (result.dtl[i]['ppn']) + '</label></td>' +
                            '<td><a onclick="$(this).parent().parent().remove();" class="btn btn-icon-only red"><i class="fa fa-times"></i></a></td>' +
                            '</tr>');
                        }
                    }
